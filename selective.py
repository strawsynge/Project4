...
class myapp(wx.App):
	def __init__(self):
		...
		self.frame=mainframe(None,"Selective Repeat")
		self.frame.Show()
...
class paintpanel(wx.Panel):
	...
	def animate(self,i):
		...
		if i==0:
			...
			for item in range(len(self.index)):
				if self.pos[item]<2*int(self.sumlen/self.lens):
					...
					# elif self.pos[item]>timeout and (self.pos[item]-timeout)%delay!=0:
					# 	if self.pos[-1]%delay==0:
					# 		if len(self.index)<n:
					# 			applist.append(self.index[-1]+1)
					# else:
		...
		tmprect=plt.Rectangle((self.index[0],-0.7),len(self.index),0.9,fill=False)
		listt.append(tmprect)
		...
		# poslist=[]
		# posiorigin=i-ndelay*delay
		# for itime in range(nuni+1):
		# 	tmp=i-(starttime+itime*delay)

		# 	if tmp<sumlen/lens:
		# 		posi=tmp
		# 		self.circle(index*n,itime,posi,'y')
		# 	if tmp==sumlen/lens:
		# 		posi=tmp
		# 		self.circle(index*n,itime,posi,'y')
		# 		tmprect=plt.Rectangle((index*n+itime,10),1,0.5,fc='r')
		# 		self.axes.add_patch(tmprect)
		# 	elif tmp>sumlen/lens and tmp<=2*sumlen/lens:
		# 		posi=2*sumlen/lens-tmp
		# 		self.circle(index*n,itime,posi,'g')
		# 	else:
		# 		posi=-1
